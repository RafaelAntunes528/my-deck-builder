        ...
        <div className="w-full bg-gradient-to-br from-gray-950/60 to-red-950/60 text-white p-6 mt-5 rounded-full shadow-2xl">
          <div className="grid grid-cols-1 gap-4">
            <div className="w-full border-2 border-red-800 p-2 rounded shadow bg-gradient-to-br from-gray-950/70 to-red-950/70 flex items-center justify-between">
              <h2 className="font-bold text-left flex items-center gap-2">
                <FaBookOpen className="text-red-400" /> Content
              </h2>
            ...
            <div className="w-full max-h-60 overflow-y-scroll border-2 border-red-800 p-4 rounded shadow flex flex-col gap-1">
              {conteudo.map(([key], index) => (
                <button
                  key={index}
                  className={`w-full text-left font-semibold px-4 py-2 rounded-lg transition duration-200 shadow border border-white/20 mb-1 ${
                    activeIndex === index
                      ? "bg-red-700 text-white scale-105"
                      : "bg-white/5 text-gray-300 hover:bg-white/15 hover:text-gray-300 hover:scale-105 hover:shadow-lg"
                  }`}
                  onClick={() => {
            ...
            <div className="w-full max-h-64 overflow-y-scroll border-2 border-red-800 p-4 rounded shadow relative bg-black/60 transition-all duration-300">
              ...
              <AnimatePresence mode="wait">
                {expanded && (
                  ...
                  >
                    {conteudo[activeIndex][1]
                      .split("\n")
                      .filter((linha) => linha.trim().length > 0)
                      .map((linha, index) => (
                        <p
                          key={index}
                          className="text-left text-base text-gray-200 font-semibold bg-white/10 border border-white/20 rounded-lg px-3 py-1 shadow-sm mb-1"
                          dangerouslySetInnerHTML={{ __html: linha.trim() }}
                        />
                      ))}
                  </motion.div>
                )}
